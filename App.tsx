...
import { 
  ...
  subscribeToBillingConfig,
  subscribeToCompany,
  subscribeToOfficeLocations,
  apiAddTask,
  apiUpdateTask,
  apiAddUser,
  apiDeleteUser,
...
import { isConfigured, resetFirebaseConfig, updateCompanyConfig } from './services/firebaseConfig';
import { 
  IconHome, 
  IconList, 
  IconUser, 
...
export default function App() {
  ...
  // Tab State
  type TabType = 'dashboard' | 'tasks' | 'team' | 'profile' | 'partners' | 'customers' | 'finance' | 'salary' | 'information' | 'inventory';
  ...
  const [companySettings, setCompanySettings] = useState<Partial<Company>>({});
  ...
  const isAppLocked = useMemo(() => {
    if (!billingConfig?.isPaywallActive || !currentCompany?.expiredAt) return false;
    const now = new Date();
    const expiry = new Date(currentCompany.expiredAt);
    return now > expiry;
  }, [billingConfig, currentCompany]);
  ...
  useEffect(() => {
    ...
    const unsubOffices = subscribeToOfficeLocations(currentUser.companyId, (locs) => {
    ...
    const unsubBilling = subscribeToBillingConfig(setBillingConfig);
    const unsubCompany = subscribeToCompany(currentUser.companyName, setCurrentCompany);

    const lastCleanup = localStorage.getItem(`lastCleanup_${currentUser.companyId}`);
    const now = Date.now();
    if (!lastCleanup || now - parseInt(lastCleanup) > 24 * 60 * 60 * 1000) {
      cleanupOldTasks(currentUser.companyId).catch(console.error);
      cleanupOldAttendance(currentUser.companyId).catch(console.error);
      cleanupOldSalaryData(currentUser.companyId).catch(console.error);
      localStorage.setItem(`lastCleanup_${currentUser.companyId}`, now.toString());
    }
  ...
  // Side effect to show paywall when locked
  useEffect(() => {
    if (isAppLocked) {
  ...
  useEffect(() => {
    if (currentUser?.companyId) {
      apiGetCompanyOnce(currentUser.companyId).then(company => {
        if (company) {
          setCompanySettings({
  ...
  const handleSaveCompanySettings = async () => {
    ...
    try {
      await apiUpdateCompanyDetails(currentUser.companyId, companySettings);
  ...
  const handleStatusClick = (task: Task) => {
    if (isAppLocked) {
      if (currentUser?.role === UserRole.DIRECTOR) {
        ...
        alert("Ứng dụng bị khóa do hết hạn sử dụng. Vui lòng liên hệ Giám đốc.");
      }
      return;
    }
  ...
  const attendanceStats = useMemo(() => {
    const stats = users.map(user => {
      ...
      const totalNC = userAtts.reduce((sum, a) => sum + (a.nc || 0), 0);
  ...
  const footerTabs = useMemo(() => {
    ...
    if (hasManagementPrivileges && !isContractor) {
    ...
    return tabs;
  }, [isWorker, isContractor, hasManagementPrivileges, isDirector, myPendingTaskCount]);

  const currentIndex = footerTabs.findIndex(t => t.id === activeTab);
  const activeTabObj = currentIndex >= 0 ? footerTabs[currentIndex] : footerTabs[0];

  const goPrev = () => {
    if (currentIndex > 0) {
      setActiveTab(footerTabs[currentIndex - 1].id);
    } else {
      setActiveTab(footerTabs[footerTabs.length - 1].id);
    }
  };

  const goNext = () => {
    if (currentIndex < footerTabs.length - 1) {
      setActiveTab(footerTabs[currentIndex + 1].id);
    } else {
      setActiveTab(footerTabs[0].id);
    }
  };

  const assignableUsers = useMemo(() => users.filter(u => u.role !== UserRole.WORKER && u.role !== UserRole.CONTRACTOR), [users]);

  if (!isSystemReady) return <SetupScreen onComplete={() => setIsSystemReady(true)} />;
  ...
  return (
    ...
    >
      ...
      {/* MAIN CONTENT */}
      <main className="flex-1 overflow-y-auto hide-scrollbar p-2 md:p-4 pb-24 w-full mx-auto relative">
        {activeTab === 'dashboard' && (
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-blue-100">
              ...
              {!isDirector && (
                <div className="flex flex-col sm:flex-row items-center gap-3">
                  ...
                  />
                  {(officeLocations.length > 0 && (!todayAttendance || !currentUser.allowPhotoCheckIn)) && (
                    <button
                      disabled={Boolean(isProcessingAttendance || isAppLocked || (todayAttendance && todayAttendance.checkOut))}
                      onClick={() => {
                        if (isAppLocked) {
                          setShowPaywall(true);
                          return;
                        }
                        if (!todayAttendance && missingCheckOuts.length > 0) {
                      ...
                      className={`flex items-center justify-center gap-2 px-8 py-3 rounded-full font-bold shadow-lg transition-all active:scale-95 w-full sm:w-auto ${
                        todayAttendance?.checkOut ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 
                        todayAttendance && !todayAttendance.checkOut ? 'bg-orange-600 hover:bg-orange-700 text-white' : 
                        'bg-brand-600 hover:bg-brand-700 text-white'
                      }`}
                    >
                      {!todayAttendance ? 'Quét QR' : 
                       !todayAttendance.checkOut ? 'Kết Thúc (Quét QR)' : 'Đã Hoàn Thành'}
                    </button>
                  )}
                  {(!officeLocations.length || currentUser.allowPhotoCheckIn) && (
                  <button
                    disabled={isProcessingAttendance || isAppLocked}
                    onClick={() => {
                      if (isAppLocked) {
                        setShowPaywall(true);
                        return;
                    ...
                    {hasManagementPrivileges ? (
                        ...
                        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-200 relative overflow-hidden">
                             ...
                             {(() => {
                                ...
                                const myNC = myAttsMonth.reduce((sum, a) => sum + (a.nc || 0), 0);
        ...
        {activeTab === 'tasks' && (
          <div className="space-y-4">
            <div className="bg-amber-50 text-amber-800 p-3 rounded-xl border border-amber-200 text-sm font-medium flex items-center justify-between">
              <span>⚠️ Toàn bộ các công việc đã Hoàn Thành quá 90 ngày sẽ bị xóa tự động khỏi hệ thống</span>
            </div>
           ...
           <div className="p-2 space-y-6 max-w-3xl mx-auto">
              <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-200 flex flex-col items-center relative overflow-hidden">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Chỉ Giám Đốc mới thấy mục quản lý nhân sự và server */}
                {isDirector && (
                  <>
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
                      <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><IconBuilding className="w-5 h-5 text-blue-500"/> Thông tin công ty (Xuất Excel)</h3>
                      <div className="space-y-4">
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">Tên công ty hiển thị (Tùy chọn)</label>
                          <input type="text" value={companySettings.exportCompanyName || ""} onChange={e => setCompanySettings({...companySettings, exportCompanyName: e.target.value})} className="w-full border border-gray-300 rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" placeholder="Ví dụ: Nội Thất Mộc Nhà Đẹp" />
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">Mã số thuế</label>
                          <input type="text" value={companySettings.taxCode || ""} onChange={e => setCompanySettings({...companySettings, taxCode: e.target.value})} className="w-full border border-gray-300 rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">Số điện thoại</label>
                          <input type="text" value={companySettings.phone || ""} onChange={e => setCompanySettings({...companySettings, phone: e.target.value})} className="w-full border border-gray-300 rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">Địa chỉ</label>
                          <textarea value={companySettings.address || ""} onChange={e => setCompanySettings({...companySettings, address: e.target.value})} className="w-full border border-gray-300 rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" rows={2} />
                        </div>
                        <button onClick={handleSaveCompanySettings} disabled={isSavingCompany} className="w-full bg-blue-600 text-white font-medium px-4 py-3 rounded-xl hover:bg-blue-700 transition-colors disabled:opacity-50">
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
                       <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><IconDatabase className="w-5 h-5 text-blue-500"/> Quản trị Máy chủ</h3>
                       <div className="space-y-3">
                         <button 
                           onClick={() => setShowUpdateConfigModal(true)}
                           className="w-full flex justify-between items-center p-4 bg-gray-50 rounded-xl hover:bg-gray-100 border border-transparent hover:border-gray-200 transition-all"
                         >
                           <div className="flex items-center gap-3"><div className="bg-blue-100 p-2 rounded-full text-blue-600"><IconDatabase className="w-5 h-5" /></div><div className="text-left"><div className="font-bold text-sm text-gray-800">Đổi cấu hình Firebase</div><div className="text-xs text-gray-500">Cập nhật nơi lưu dữ liệu</div></div></div>
                         </button>
                       </div>
                    </div>
                  </>
        ...
        {activeTab === 'salary' && (
          <div className="space-y-4">
            {isDirector && (
              <div className="bg-red-50 text-red-800 p-3 rounded-xl border border-red-200 text-sm font-medium flex items-center justify-between">
                <span>⚠️ Giám đốc lưu ý: Dữ liệu tính lương (kèm ảnh chấm công) quá 90 ngày sẽ bị xóa tự động.</span>
              </div>
            )}
            {isWorker && (
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-blue-100">
        ...
        {activeTab === 'inventory' && isDirector && (
          <InventoryTab currentUser={currentUser} isLocked={isAppLocked} />
        )}
      ...
      {hasManagementPrivileges && (activeTab === 'tasks' || activeTab === 'dashboard') && (
      ...
      {/* FOOTER NAV */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-20 pb-safe shadow-[0_-5px_10px_rgba(0,0,0,0.02)]">
        {/* Mobile View */}
        <div className="flex md:hidden items-center justify-between px-4 py-3">
          <button onClick={goPrev} className="p-3 text-gray-500 hover:text-blue-600 rounded-xl hover:bg-blue-50 transition-colors shrink-0 active:bg-blue-100">
            <IconChevronLeft className="w-8 h-8" />
          </button>
          <div className="flex-1 flex justify-center items-center">
             {activeTabObj && (
               <NavButton active={true} onClick={() => {}} icon={activeTabObj.icon} label={activeTabObj.label} badge={activeTabObj.badge} />
             )}
          </div>
          <button onClick={goNext} className="p-3 text-gray-500 hover:text-blue-600 rounded-xl hover:bg-blue-50 transition-colors shrink-0 active:bg-blue-100">
            <IconChevronRight className="w-8 h-8" />
          </button>
        </div>
        {/* Desktop View */}
        <div className="hidden md:flex px-6 py-3 justify-between items-center overflow-x-auto">
          {footerTabs.map(tab => (
             <NavButton key={tab.id} active={activeTab === tab.id} onClick={() => setActiveTab(tab.id)} icon={tab.icon} label={tab.label} badge={tab.badge} />
          ))}
        </div>
      </nav>

      {/* MODALS */}
      {showCreateModal && <CreateTaskModal currentUser={currentUser} assignableUsers={assignableUsers} onClose={() => setShowCreateModal(false)} onCreate={createTask} isLocked={isAppLocked} />}
      {editingTask && <CreateTaskModal currentUser={currentUser} assignableUsers={assignableUsers} onClose={() => setEditingTask(null)} onCreate={async () => {}} onEdit={submitEditTask} initialData={editingTask} isLocked={isAppLocked} />}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
          <div className="bg-white w-full max-w-md rounded-2xl p-6 shadow-2xl">
            <h3 className="text-xl font-bold text-navy-800 mb-2 flex items-center gap-2">
              <IconDatabase className="w-6 h-6 text-blue-500" /> Cập nhật cấu hình Firebase
            </h3>
            <p className="text-sm text-gray-500 mb-6">
              Nhập cấu hình JSON mới cho công ty <strong>{currentUser.companyName}</strong>. Dữ liệu cũ sẽ không bị mất, ứng dụng chỉ chuyển hướng kết nối sang Firebase mới.
            </p>
            
            <div className="space-y-4 mb-6">
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Cấu hình JSON mới</label>
                <textarea
                  rows={6}
                  value={newConfigJson}
                  onChange={(e) => setNewConfigJson(e.target.value)}
                  placeholder='{ "apiKey": "...", "projectId": "..." }'
                  className="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-navy-800 outline-none font-mono text-xs"
                />
              </div>
            </div>
              ...
              <button 
                onClick={async () => {
                  if (!newConfigJson.trim()) {
                    alert('Vui lòng nhập cấu hình JSON');
                    return;
                  }
                  setIsUpdatingConfig(true);
                  try {
                    await updateCompanyConfig(currentUser.companyName, newConfigJson);
                    alert('Cập nhật cấu hình thành công! Ứng dụng sẽ tải lại.');
                ...
                disabled={isUpdatingConfig || !newConfigJson.trim()} 
                className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-lg disabled:opacity-50 transition-colors flex justify-center items-center"
              >
      ...
      {showQRCheckIn && (
        <QRCheckIn 
          ...
          onCancel={() => setShowQRCheckIn(false)}
          onCheckIn={async (location) => {
            setShowQRCheckIn(false);
            
            // Re-use logic from check-in with camera, but we don't have an image
            setIsProcessingAttendance(true);
            try {
              const now = new Date();
              ...
              if (!todayAttendance) {
                ...
                const newAtt: Attendance = {
                  ...
                  tc: 0,
                  checkIn: timeStr,
                  checkInImage: '', // No photo needed via QR
                  note: `Chấm công QR: ${location.name}`,
                  otHours: 0,
                  otStatus: OTStatus.NONE,
                  flag: AttendanceFlag.NORMAL
                };
                ...
                await apiAddAttendance(newAtt);
                alert(`Check-in QR thành công lúc ${timeStr}.`);
              } else {
                // Check-out
                const updatedAtt: Attendance = {
                  ...todayAttendance,
                  checkOut: timeStr,
                  checkOutImage: '',
                  note: todayAttendance.note ? `${todayAttendance.note} | Checkout QR: ${location.name}` : `Checkout QR: ${location.name}`
                };
                ...
                await apiUpdateAttendance(updatedAtt);
                alert(`Check-out QR thành công lúc ${timeStr}.`);
              }
            } catch (error) {
              console.error("QR Check-in/out failed:", error);
              alert("Gặp lỗi khi lưu chấm công.");
            } finally {
              setIsProcessingAttendance(false);
            }
          }}
