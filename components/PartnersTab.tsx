        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="relative">
            ...
            <input 
              ...
              className="pl-9 pr-4 py-1.5 border border-gray-200 rounded-lg text-sm outline-none focus:border-blue-500 w-48 md:w-64"
            />
          </div>
  ...
  const renderDetailView = () => {
    ...
    const exportDetailToExcel = () => {
      const data = filteredTx.map((t, index) => {
        const product = products.find(p => p.id === t.productId) || financeCategories.find(c => c.id === t.productId) || (t.productId === 'inventory_export' ? { name: 'Xuất kho' } : (t.productId === 'inventory_import' ? { name: 'Nhập kho' } : { name: 'Chưa xác định' }));
        return {
          'STT': index + 1,
    ...
    <div className="p-2 md:p-4 w-full mx-auto space-y-6 relative pb-24">
      ...
      <MobileTabNavigation
        tabs={[
          ...
          { id: 'partners', label: 'Danh mục đối tác' },
          { id: 'products', label: 'Danh mục hàng hóa' }
        ]}
...
function AddTransactionModal({ onClose, currentUser, partners, products, onAddPartner, onAddProduct, initialData }: any) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-fade-in flex flex-col max-h-[90dvh]">
