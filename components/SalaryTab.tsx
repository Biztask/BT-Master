...
const Payslip = ({ data, month, companyName, onClose, onPrint }: { data: any, month: string, companyName: string, onClose?: () => void, onPrint?: () => void }) => {
  ...
  return (
    <div className="p-4 sm:p-8 bg-white text-black w-full mx-auto relative page-break rounded-2xl print:shadow-none print:border-none print:rounded-none print:m-0 print:p-0">
      <div className="absolute top-4 right-4 flex gap-2 print:hidden">
        {onPrint && (
          <button onClick={onPrint} className="w-8 h-8 flex items-center justify-center bg-indigo-50 text-indigo-600 rounded-full hover:bg-indigo-100 transition-colors">
            <IconPrinter className="w-4 h-4" />
          </button>
        )}
        {onClose && (
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center bg-gray-50 text-gray-600 rounded-full hover:bg-gray-100 transition-colors">
            <IconX className="w-4 h-4" />
          </button>
        )}
      </div>

      <div className="text-center mb-6 border-b border-gray-300 pb-6 pt-2">
        <h1 className="text-2xl font-black uppercase tracking-wider text-gray-900">{companyName}</h1>
        <h2 className="text-xl font-bold mt-2 text-gray-800">PHIẾU BÁO LƯƠNG</h2>
        <p className="text-sm text-gray-500 mt-1 font-medium">Tháng {formattedMonth}</p>
      </div>
      
      <div className="mb-6 bg-gray-50 p-4 rounded-xl border border-gray-100 flex flex-col sm:flex-row justify-between text-sm">
        <div className="flex gap-4 mb-2 sm:mb-0 w-full sm:w-1/2">
          <span className="text-gray-500 w-24 shrink-0">Họ và tên:</span>
          <span className="font-bold text-gray-900 truncate">{data.employee.name}</span>
        </div>
        <div className="flex flex-col gap-2 w-full sm:w-1/2 sm:pl-8">
          <div className="flex gap-4">
            <span className="text-gray-500 w-24 shrink-0">Bộ phận:</span>
            <span className="font-medium text-gray-900 truncate">{data.employee.department || 'N/A'}</span>
          </div>
          <div className="flex gap-4">
            <span className="text-gray-500 w-24 shrink-0">Ngày in:</span>
            <span className="font-medium text-gray-900 truncate">{new Date().toLocaleDateString('vi-VN')}</span>
          </div>
        </div>
      </div>

      <div className="mb-6">
        <h3 className="font-bold text-gray-800 mb-3 uppercase tracking-wide text-xs">Chi Tiết Thu Nhập</h3>
        <table className="w-full text-sm">
          <tbody className="divide-y divide-gray-100">
            <tr>
              <td className="py-2 text-gray-600">Lương cơ bản (tháng)</td>
              <td className="py-2 text-right font-medium">{formatCurrency(data.info?.monthlySalary || 0)}</td>
            </tr>
            <tr>
              <td className="py-2 text-gray-600">Lương ngày chuẩn</td>
              <td className="py-2 text-right font-medium">{formatCurrency(data.dailySalary || 0)}</td>
            </tr>
            <tr>
              <td className="py-2 text-gray-600">Số ngày công thực tế (NC)</td>
              <td className="py-2 text-right font-medium">{data.totalNC} ngày</td>
            </tr>
            <tr>
              <td className="py-2 text-gray-900 font-bold">Tổng lương ngày công</td>
              <td className="py-2 text-right font-bold text-gray-900">{formatCurrency(data.salaryNC)}</td>
            </tr>
            <tr>
              <td className="py-2 text-gray-600">Số giờ tăng ca quy đổi (TC)</td>
              <td className="py-2 text-right font-medium">{data.totalTC} ngày</td>
            </tr>
            <tr>
              <td className="py-2 text-gray-900 font-bold">Tổng lương tăng ca</td>
              <td className="py-2 text-right font-bold text-gray-900">{formatCurrency(data.salaryTC)}</td>
            </tr>
            <tr>
              <td className="py-2 text-gray-600">Phụ cấp khác</td>
              <td className="py-2 text-right font-medium">{formatCurrency(payroll.allowance || 0)}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div className="mb-6">
        <h3 className="font-bold text-gray-800 mb-3 uppercase tracking-wide text-xs">Các khoản khấu trừ & điều chỉnh</h3>
        <table className="w-full text-sm">
          <tbody className="divide-y divide-gray-100">
            <tr>
              <td className="py-2 text-gray-600">Tạm ứng trong tháng</td>
              <td className="py-2 text-right font-medium text-red-500">-{formatCurrency(data.totalAdvance)}</td>
            </tr>
            <tr>
              <td className="py-2 text-gray-600">Đã thanh toán trước</td>
              <td className="py-2 text-right font-medium text-red-500">-{formatCurrency(payroll.paidAmount || 0)}</td>
            </tr>
            <tr>
              <td className="py-2 text-gray-600">Dư nợ/có tháng trước chuyển sang</td>
              <td className="py-2 text-right font-medium text-blue-500">{formatCurrency(payroll.previousBalance || 0)}</td>
            </tr>
          </tbody>
        </table>
      </div>

      <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl flex justify-between items-center mb-8">
        <span className="text-base font-bold text-blue-900 uppercase">Thực lĩnh kỳ này</span>
        <span className="text-2xl font-black text-blue-700">{formatCurrency(data.finalSalary)}</span>
      </div>

      <div className="grid grid-cols-2 gap-4 mt-8 pb-4 text-center">
        <div>
          <p className="font-bold text-gray-800 text-sm">Người lập phiếu</p>
          <p className="text-xs text-gray-400 mt-1 italic">(Ký, ghi rõ họ tên)</p>
          <div className="h-16"></div>
        </div>
        <div>
          <p className="font-bold text-gray-800 text-sm">Người nhận</p>
          <p className="text-xs text-gray-400 mt-1 italic">(Ký, ghi rõ họ tên)</p>
          <div className="h-16"></div>
        </div>
      </div>
    ...
    <div className="flex flex-col h-full bg-gray-50 overflow-hidden">
      ...
      <MobileTabNavigation
        tabs={[
          ...
          ...(currentUser.role !== UserRole.WORKER ? [{ id: 'advance', label: 'Tạm ứng' }] : []),
          { id: 'payroll', label: 'Bảng lương' },
          ...(hasManagementPrivileges ? [{ id: 'config', label: 'Cấu hình' }] : [])
        ]}
      ...
      <div className="flex-1 overflow-hidden p-4 flex flex-col min-h-0">
        ...
        {activeSubTab === 'config' && editConfig && (
          <div className="max-w-2xl mx-auto bg-white p-6 rounded-xl shadow-sm border space-y-6">
            ...
            <div className="pt-4 pb-10 sm:pb-4 border-t flex justify-end">
              <button 
      ...
      {showAttendanceDetail && viewingAttendance && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[70] p-2 pb-24 sm:p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl max-h-[75dvh] sm:max-h-[90vh] flex flex-col overflow-hidden w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="p-4 pb-10 sm:pb-4 bg-gray-50 border-t flex justify-end">
              <button 
      ...
      {/* Add Advance Modal */}
      {showAddAdvance && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[70] p-2 pb-24 sm:p-4">
          <div className="bg-white rounded-2xl p-6 max-h-[75dvh] sm:max-h-[90vh] flex flex-col w-full max-w-md shadow-2xl">
      ...
      {showSignatureConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-2 pb-24 sm:p-4 z-[70] animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 max-h-[75dvh] sm:max-h-[90vh] flex flex-col">
      ...
      {viewingPayslip && (
        <div className="fixed inset-0 bg-black/60 z-[100] flex flex-col items-center justify-center p-2 sm:p-4 backdrop-blur-sm animate-fade-in print:hidden">
          <div className="w-full max-w-xl shadow-2xl overflow-y-auto max-h-[90dvh] rounded-2xl bg-white relative">
            <Payslip 
              data={viewingPayslip} 
              month={selectedMonth} 
