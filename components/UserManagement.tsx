...
const UserManagement: React.FC<UserManagementProps> = ({ currentUser, companyUsers, onAddUser, onDeleteUser }) => {
  ...
  const [newAllowPhotoCheckIn, setNewAllowPhotoCheckIn] = useState(false);
  ...
  const [editAllowPhotoCheckIn, setEditAllowPhotoCheckIn] = useState(false);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    const newUser: User = {
      ...
      allowPhotoCheckIn: newAllowPhotoCheckIn,
  ...
  const handleEditClick = (user: User) => {
    ...
    setEditAllowPhotoCheckIn(user.allowPhotoCheckIn || false);
  ...
  const confirmEdit = async () => {
    if (userToEdit && editName) {
      const updatedUser = { 
        ...
        allowPhotoCheckIn: editAllowPhotoCheckIn,
        role: editRole
      };
    ...
    <div className="space-y-4">
      ...
      {showAddForm && (
        <form onSubmit={handleAdd} className="bg-gray-50 p-4 rounded-xl border border-gray-200 mb-4 space-y-3 animate-fade-in relative">
          ...
          <div>
            <label className="text-xs text-gray-500 font-bold mb-2 block">Hình thức chấm công</label>
            <label className="flex items-center gap-3 p-3 rounded-xl border border-gray-200 bg-white cursor-pointer hover:bg-gray-50 transition-all">
              <div className="relative inline-block w-10 mr-2 align-middle select-none transition duration-200 ease-in">
                <input 
                  type="checkbox" 
                  name="allowPhotoCheckIn" 
                  id="allowPhotoCheckInAdd" 
                  checked={newAllowPhotoCheckIn}
                  onChange={(e) => setNewAllowPhotoCheckIn(e.target.checked)}
                  className="toggle-checkbox absolute block w-5 h-5 rounded-full bg-white border-4 appearance-none cursor-pointer disabled:cursor-not-allowed"
                  disabled={isProcessing}
                  style={{
                    right: newAllowPhotoCheckIn ? '0' : '1.25rem',
                    borderColor: newAllowPhotoCheckIn ? '#10B981' : '#D1D5DB',
                  }}
                />
                <label 
                  htmlFor="allowPhotoCheckInAdd" 
                  className={`toggle-label block overflow-hidden h-5 rounded-full cursor-pointer ${newAllowPhotoCheckIn ? 'bg-green-500' : 'bg-gray-300'}`}
                ></label>
              </div>
              <div className="flex flex-col">
                <span className="font-bold text-sm text-gray-800">
                  {newAllowPhotoCheckIn ? 'QR + Chụp Ảnh' : 'Chỉ mã QR'}
                </span>
                <span className="text-[10px] text-gray-500 mt-0.5">
                  {newAllowPhotoCheckIn ? 'Nhân viên có thể chấm công bằng QR hoặc chụp hình' : 'Bắt buộc nhân viên quét mã QR tại văn phòng'}
                </span>
              </div>
            </label>
          </div>
          ...
          <div>
            <label className="text-xs text-gray-500 font-bold mb-2 block">Chức vụ & Quyền hạn</label>
            <div className="grid grid-cols-1 gap-2">
              {[
                { 
                  role: UserRole.DIRECTOR, 
                  label: 'Quản trị máy chủ', 
                  color: 'red',
                  desc: 'Toàn quyền điều hành, xem tất cả dữ liệu tài chính và nhân sự.'
                },
                { 
                  role: UserRole.MANAGER, 
                  label: 'Quản lý', 
                  color: 'purple',
                  desc: 'Nhập dữ liệu, xem thông tin như Giám đốc nhưng ở TAB Lương chỉ được xem, không được thao tác.'
                },
                { 
                  role: UserRole.EMPLOYEE, 
                  label: 'Nhân viên', 
                  color: 'blue',
                  desc: 'Chấm công, nhận và hoàn tất công việc, phản hồi lương.'
                },
                { 
                  role: UserRole.WORKER, 
                  label: 'Công nhân', 
                  color: 'orange',
                  desc: 'Chấm công, phản hồi lương, không giao việc.'
                },
                { 
                  role: UserRole.CONTRACTOR, 
                  label: 'NC Khoán', 
                  color: 'teal',
                  desc: 'Quản lý nhân sự khoán, không giao việc, không tính lương.'
                },
              ].map((item) => (
                <label 
                  key={item.role}
                  className={`flex flex-col p-3 rounded-xl border transition-all cursor-pointer ${
                    newRole === item.role 
                      ? `border-${item.color}-500 bg-${item.color}-50 ring-1 ring-${item.color}-500` 
                ...
                >
                  <div className="flex items-center gap-3">
                    <input 
                      ...
                      disabled={isProcessing}
                      className={`text-${item.color}-600 focus:ring-${item.color}-500`}
                    />
                    <span className={`font-bold text-sm text-${item.color}-700`}>{item.label}</span>
                  </div>
                  <p className="text-[10px] text-gray-500 mt-1 ml-7 leading-tight">{item.desc}</p>
                </label>
              ))}
            </div>
          </div>
              ...
              <div className="min-w-0 pr-2">
                ...
                <div className="text-xs text-gray-500 truncate">
                  ...
                  {currentUser.role === UserRole.DIRECTOR && (
                    <span className="ml-2 text-gray-400">| Mật khẩu: <span className="font-mono text-gray-600">{user.password}</span></span>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
          <div className="bg-white w-full max-w-sm rounded-2xl p-6 shadow-2xl animate-slide-up space-y-4">
             ...
             <div className="space-y-3">
                ...
                <div>
                  <label className="text-xs text-gray-500 font-bold mb-1 block">Mật khẩu</label>
                  <input 
                    type="text"
                    value={editPassword}
                ...
                <div>
                  ...
                  <label className="flex items-center gap-3 p-3 rounded-xl border border-gray-200 bg-white cursor-pointer hover:bg-gray-50 transition-all">
                    <div className="relative inline-block w-10 mr-2 align-middle select-none transition duration-200 ease-in">
                      <input 
                        type="checkbox" 
                        id="allowPhotoCheckInEdit" 
                        checked={editAllowPhotoCheckIn}
                        onChange={(e) => setEditAllowPhotoCheckIn(e.target.checked)}
                        className="toggle-checkbox absolute block w-5 h-5 rounded-full bg-white border-4 appearance-none cursor-pointer"
                        style={{
                          right: editAllowPhotoCheckIn ? '0' : '1.25rem',
                          borderColor: editAllowPhotoCheckIn ? '#10B981' : '#D1D5DB',
                        }}
                      />
                      <label 
                        htmlFor="allowPhotoCheckInEdit" 
                        className={`toggle-label block overflow-hidden h-5 rounded-full cursor-pointer ${editAllowPhotoCheckIn ? 'bg-green-500' : 'bg-gray-300'}`}
                      ></label>
                    </div>
                    <div className="flex flex-col">
                      <span className="font-bold text-sm text-gray-800">
                        {editAllowPhotoCheckIn ? 'QR + Chụp Ảnh' : 'Chỉ mã QR'}
                      </span>
                      <span className="text-[10px] text-gray-500 mt-0.5">
                        {editAllowPhotoCheckIn ? 'Nhân viên có thể chấm công bằng QR hoặc chụp hình' : 'Bắt buộc nhân viên quét mã QR tại văn phòng'}
                      </span>
                    </div>
                  </label>
                </div>
      ...
      {(currentUser.role === UserRole.DIRECTOR) && (
        <OfficeManagement companyId={currentUser.companyId} />
      )}
    </div>
  );
};
