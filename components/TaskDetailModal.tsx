...
const TaskDetailModal: React.FC<TaskDetailModalProps> = ({ task, onClose, users, onEdit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
      <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90dvh] overflow-hidden">
      ...
      {lightboxImg && (
         <div className="fixed inset-0 z-[60] bg-black/95 flex items-center justify-center p-4 animate-fade-in" onClick={(e) => { e.stopPropagation(); setLightboxImg(null); }}>
            ...
            <img src={lightboxImg} className="max-w-full max-h-[90dvh] object-contain rounded-lg shadow-2xl" onClick={(e) => e.stopPropagation()} />
         </div>
      )}
    </div>
  );
